...
const calculateTotalSavedForEntry = (entry: SavingsEntry): number => {
  if (!entry || !entry.startDate || typeof entry.weeklyAmount !== 'number')
    return 0
  try {
...
export default function SavingsCalculator () {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const tx = db.transaction(
          [STORE_NAMES.SAVINGS, STORE_NAMES.EXPENSES],
          'readonly'
        )
        ...
        const validEntries = entries.filter(
          e =>
            e &&
            typeof e.weeklyAmount === 'number' &&
            !isNaN(e.weeklyAmount) &&
            e.startDate
        )
        setSavingsEntries(validEntries)

        let calculatedTotalExpenses = 0
        let calculatedScholarshipTotal = 0
        allBudgetCategories.forEach(cat => {
          ...
          if (cat.category === 'Scholarships' || cat.type === 'asset') {
            calculatedScholarshipTotal += categoryTotal
          } else if (cat.type === 'expense') {
            calculatedTotalExpenses += categoryTotal
          }
        })
        setTotalCost(calculatedTotalExpenses)
        setFundsAvailable(calculatedScholarshipTotal)
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const saved = savingsEntries.reduce(
      (a, e) => a + calculateTotalSavedForEntry(e),
      0
    )
    ...
    if (weekly <= 0 || remain <= 0) {
      setTimeEstimate(
        remain <= 0 && totalCost > 0 ? 'Goal Reached!' : 'Add contributions'
      )
    } else {
      const w = Math.ceil(remain / weekly)
      const y = Math.floor(w / 52)
      const m = Math.floor((w % 52) / 4)
      const rw = w % 4
      let p: string[] = []
      if (y > 0) p.push(`${y} yr${y > 1 ? 's' : ''}`)
      if (m > 0) p.push(`${m} mo${m > 1 ? 's' : ''}`)
      if (rw > 0) p.push(`${rw} wk${rw > 1 ? 's' : ''}`)
      setTimeEstimate(p.length > 0 ? p.join(' ') : '< 1 wk')
    }

    if (totalCost <= 0) {
      setFundingPercentage(fundsAvailable + saved > 0 ? 100 : 0)
    } else {
      const perc = ((fundsAvailable + saved) / totalCost) * 100
      setFundingPercentage(Math.max(0, Math.min(perc, 100)))
    }
  ...
  const handleAddSavings = useCallback(async () => {
    setError(null)
    if (!contributionName.trim() || !weeklyAmount || !startDate) {
    ...
    try {
      if (!isValid(parseISO(startDate))) throw new Error('Invalid date')
    } catch (e) {
    ...
    try {
      ...
      console.error('Add Save Err:', err)
      setError('Failed add.')
    }
  ...
  const handleRemoveSavings = useCallback(async (id: number) => {
    ...
    try {
      ...
      console.error('Remove Save Err:', err)
    ...
    <Layout backgroundImageSrc={background}>
      <div className='relative min-h-screen'>
        ...
        <div className='max-w-4xl mx-auto p-6 backdrop-blur-lg rounded-xl shadow-lg bg-white/95 text-gray-900 dark:bg-gray-900/90 dark:text-gray-100'>
          ...
          {error && (
            <div className='mb-4 p-3 bg-red-100 dark:bg-red-900/30 border border-red-400 dark:border-red-600 text-red-700 dark:text-red-300 rounded text-center'>
              {error}
            </div>
          )}
          ...
          <Card className='mb-6 bg-gray-50 border-gray-200 dark:bg-gray-800 dark:border-gray-700'>
            ...
            <CardContent className='space-y-2 text-sm'>
              <p className='text-gray-600 dark:text-gray-300'>
                1. Budget data (Total Cost, Scholarships) is pulled from your
                Budget Sheet.
              </p>
              <p className='text-gray-600 dark:text-gray-300'>
                2. Add your weekly savings contributions below.
              </p>
              <p className='text-gray-600 dark:text-gray-300'>
                3. See progress, total saved (based on start dates), and time to
                goal.
              </p>
              <p className='text-gray-600 dark:text-gray-300'>
                4. Update budget via the{' '}
                <Button
                  variant='link'
          ...
          <Card className='mb-6 bg-white border-gray-200 dark:bg-gray-800 dark:border-gray-700'>
            ...
            <CardContent>
              <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4'>
                <div className='text-center p-3 rounded bg-gray-100/80 dark:bg-gray-700/60'>
                  ...
                  <div className='text-2xl font-bold text-red-600 dark:text-red-400'>
                    $
                    {Math.max(
                      totalCost - fundsAvailable - totalSaved,
                      0
                    ).toFixed(2)}
                  </div>
                </div>
              ...
              <div className='space-y-1 text-sm mb-4 text-gray-700 dark:text-gray-300'>
                ...
                <p>
                  <span className='font-semibold'>Funds/Scholarships:</span> $
            ...
            <CardContent>
              ...
              <div className='grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-6 mb-4'>
                ...
                <div>
                  ...
                  />
                  <p
                    id='contribution-name-description'
                    className='text-xs text-gray-500 dark:text-gray-400 mt-1 px-1'
                  >
                    Enter a name for this savings source (e.g., Paycheck, Side
                    Job).
                  </p>
                </div>
                <div>
              ...
              {/* === MODIFICATION END === */}
              <Button onClick={handleAddSavings} className='w-full md:w-auto'>
                Add Contribution
              </Button>
            </CardContent>
          </Card>
                ...
                <div className='space-y-3'>
                  {savingsEntries.map(entry => {
                    ...
                    return (
                      <div
                        key={entry.id}
                        className='flex justify-between items-center p-3 rounded-lg shadow-sm bg-gray-50 text-gray-900 dark:bg-gray-700 dark:text-gray-100'
                      >
                        <div>
                          ...
                          <span className='text-sm block text-gray-700 dark:text-gray-300'>
                            ${entry.weeklyAmount?.toFixed(2) || '0.00'} / week
                          </span>
                          {isValid(startDateObj) && (
                            <span className='text-xs opacity-80 block dark:opacity-70'>
                              Since: {format(startDateObj, 'MMM dd, yy')}
                            </span>
                          )}{' '}
                          {/* Shortened year */}
                          <span className='text-xs opacity-80 block dark:opacity-70'>
                            Total Saved: ${savedForEntry.toFixed(2)}
                          </span>
                        </div>
                        <Button
                          onClick={() => handleRemoveSavings(entry.id)}
                          variant='ghost'
                          size='sm'
                          className='text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50 px-2 py-1 h-auto'
                        >
                          Remove
                        </Button>
                      </div>

...
const addEventToDB = async (newEvent: CalendarEvent): Promise<boolean> => {
  try {
    const d = await getDb()
    const t = d.transaction(STORE_NAMES.CALENDAR, 'readwrite')
    await t.objectStore(STORE_NAMES.CALENDAR).put(newEvent)
    await t.done
    return true
  } catch (e) {
    console.error('Add Ev Err:', e)
    return false
  }
}
const updateEventInDB = async (
  ...
  try {
    const d = await getDb()
    const t = d.transaction(STORE_NAMES.CALENDAR, 'readwrite')
    await t.objectStore(STORE_NAMES.CALENDAR).put(updatedEvent)
    await t.done
    return true
  } catch (e) {
    console.error('Upd Ev Err:', e)
    return false
  }
}
const removeEventFromDB = async (eventId: string): Promise<boolean> => {
  try {
    const d = await getDb()
    const t = d.transaction(STORE_NAMES.CALENDAR, 'readwrite')
    await t.objectStore(STORE_NAMES.CALENDAR).delete(eventId)
    await t.done
    return true
  } catch (e) {
    console.error('Rem Ev Err:', e)
    return false
  }
}
const getAllCalendarEvents = async (): Promise<CalendarEvent[]> => {
  try {
    const d = await getDb()
    const t = d.transaction(STORE_NAMES.CALENDAR, 'readonly')
    const s = t.objectStore(STORE_NAMES.CALENDAR)
    const ev = await s.getAll()
    await t.done
    return ev.filter(e => e.id !== 'predefinedEventsInitialized')
  } catch (e) {
...
export default function CalendarPage () {
  ...
  useEffect(() => {
    ...
    const initializeAndFetchEvents = async () => {
      ...
      try {
        ...
        console.error('Init/Fetch Err:', err)
  ...
  const handleEventChange = useCallback(
    async (changeInfo: any /* Use type from FullCalendar */) => {
      ...
      if (success) {
        ...
        setError('Failed to update event.') /* Optionally revert change */
      }
    },
    []
  )
        ...
        <DialogContent className='sm:max-w-[425px] bg-white dark:bg-gray-800'>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className='font-medium text-gray-700 dark:text-gray-300'>
                {formatModalDateRange(addModalData)}
              </span>
            </DialogDescription>
          </DialogHeader>
          <div className='grid gap-4 py-4'>
            ...
            {error && isAddModalOpen && (
              <p className='col-span-4 text-sm text-red-500 dark:text-red-400 text-center'>
                {error}
              </p>
            )}
          </div>
          <DialogFooter>
            ...
            <Button type='button' onClick={handleAddEventConfirm}>
              Save Event
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <div className='mb-6'>
        <label className='flex items-center space-x-2 cursor-pointer'>
          <input
            ...
            className='form-checkbox h-4 w-4 rounded text-blue-600 border-gray-300 dark:text-blue-400 dark:bg-gray-600 dark:border-gray-500 focus:ring-blue-500 dark:focus:ring-blue-400'
          />

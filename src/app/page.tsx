...
const getAllDashboardData = async (): Promise<{ deadlines: CalendarEvent[], budgetProgress: BudgetProgressData }> => {
    let deadlines: CalendarEvent[] = [];
    let budgetProgressData: BudgetProgressData = { totalBudget: 0, totalSaved: 0, scholarshipTotal: 0, progress: 0 };

    try {
        const db = await getDb();
        // Ensure all types match the stores defined in db.ts
        const tx = db.transaction([STORE_NAMES.CALENDAR, STORE_NAMES.EXPENSES, STORE_NAMES.SAVINGS], 'readonly');
        ...
        const [allEvents, allExpenses, allSavings] = await Promise.all([
            ...
            tx.objectStore(STORE_NAMES.EXPENSES).getAll() as Promise<ExpenseCategory[]>,
        ...
        deadlines = allEvents
            .filter(event => {
                ...
                try {
                    ...
                    return isValid(deadline) && deadline >= todayStart && deadline <= futureDateLimit;
                } catch { return false; }
            })
            .sort((a, b) => {
                try {
        ...
        const totalExpenses = allExpenses
            ...
            .reduce((total, cat) => {
                ...
                return total + items.reduce((sum, item) => sum + (Number(item?.cost) || 0), 0);
            }, 0);

        // Use defined constant for Scholarship category
        const scholarships = allExpenses.find(cat => cat?.category === SCHOLARSHIP_CATEGORY)?.items || [];
        const scholarshipTotal = scholarships.reduce((sum, item) => sum + (Number(item?.cost) || 0), 0);

        const calculateTotalSavedForEntry = (entry: SavingsEntry): number => {
            // Add basic check for entry and weeklyAmount validity
             if (!entry || typeof entry.weeklyAmount !== 'number' || entry.weeklyAmount < 0) return 0;
             // Check startDate validity
            if (!entry.startDate || typeof entry.startDate !== 'string') return entry.weeklyAmount; // Fallback: return one week if no valid start date
            ...
            } catch { return 0; } // Return 0 on parsing error
        };

        const totalSavedFromSavings = allSavings.reduce((sum, entry) => sum + calculateTotalSavedForEntry(entry), 0);
        ...
        budgetProgressData = {
            ...
            progress: totalExpenses > 0 ? Math.min(100, (totalFundsAvailable / totalExpenses) * 100) : (totalFundsAvailable > 0 ? 100 : 0)
        };
...
const DeadlineItem: React.FC<{ event: CalendarEvent }> = ({ event }) => {
    ...
    return (
        <li key={event.id} className="p-3 rounded-lg bg-gray-100 hover:bg-gray-200/80 dark:bg-gray-700 dark:hover:bg-gray-600/80 transition-colors text-gray-900 dark:text-gray-100">
            <div className="font-semibold text-base">{event.title || 'Untitled Event'}</div>
            <div className="text-sm opacity-90 dark:opacity-80">{formattedStart}{formattedEnd}</div>
            {daysRemainingText && <div className="text-xs opacity-75 mt-1">{daysRemainingText}</div>}
        </li>
    );
};
...
export default function HomeScreen() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const { deadlines, budgetProgress: progressData } = await getAllDashboardData();
                ...
                setBudgetProgress({ totalBudget: 0, totalSaved: 0, scholarshipTotal: 0, progress: 0 }); // Reset on error
            } finally {
                setLoading(false);
            }
        };
        ...
        <Layout backgroundImageSrc={background}>
            ...
            <div className="relative z-0"> {/* Added min-h-screen here ensures content area can fill viewport height if needed */}
                ...
                <div className="max-w-7xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {loading ? (
                            ...
                            <div className="flex flex-col md:flex-row items-center gap-6">
                                ...
                                <div className="space-y-2 text-center md:text-left text-sm">
                                     ...
                                     <p className="font-bold text-gray-900 dark:text-gray-100"><span className="text-gray-600 dark:text-gray-400 font-medium">Total Funds:</span><span className="ml-2">${budgetProgress.totalSaved.toFixed(2)}</span></p>
                                     <p className={`font-bold ${remainingBudget > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                                        <span className="text-gray-600 dark:text-gray-400 font-medium">Remaining Need:</span>
                                        <span className="ml-2">${Math.max(0, remainingBudget).toFixed(2)}</span>
                                        {remainingBudget < 0 && ( <span className="ml-2 text-xs font-medium">(Surplus: ${(-remainingBudget).toFixed(2)})</span> )}
                                    </p>
                                    <Button onClick={() => navigateTo('/budget')} size="sm" className="mt-2">
                                        View Budget Details
                                    </Button>
                                     <Button onClick={() => navigateTo('/savings')} size="sm" className="mt-2 ml-2">
                                         View Savings Plan
                                     </Button>
                                </div>
                            </div>
                    ...
                    <div className="backdrop-blur-lg rounded-xl shadow-lg p-6 bg-white/95 dark:bg-gray-800/90">
                        <h1 className="text-2xl lg:text-3xl font-bold mb-4 text-center text-gray-900 dark:text-gray-100">Plan Your Study Abroad Journey</h1>
                        <p className="text-base lg:text-lg mb-6 text-center text-gray-700 dark:text-gray-300">
                            Use these tools to manage funding, track deadlines, and stay organized:
                        </p>
                        <ul className="space-y-4 text-sm lg:text-base">
                            <li className="flex items-start">
                                <span className="text-blue-600 dark:text-blue-400 font-bold text-xl mr-2 mt-[-2px]">•</span>
                                <p className="text-gray-800 dark:text-gray-200"><strong className="font-semibold">Explore Scholarships:</strong> Browse opportunities and track your application status.</p>
                            </li>
                             <Button onClick={() => navigateTo('/scholarshipplan')} variant="link" className="text-blue-600 dark:text-blue-400 h-auto p-0 ml-5 text-sm lg:text-base">Go to Scholarship Planner →</Button>

                            <li className="flex items-start mt-3">
                                <span className="text-blue-600 dark:text-blue-400 font-bold text-xl mr-2 mt-[-2px]">•</span>
                                <p className="text-gray-800 dark:text-gray-200"><strong className="font-semibold">Track Deadlines:</strong> Add important dates to the calendar so you never miss out.</p>
                            </li>
                            <Button onClick={() => navigateTo('/calendar')} variant="link" className="text-blue-600 dark:text-blue-400 h-auto p-0 ml-5 text-sm lg:text-base">Go to Calendar →</Button>

                            <li className="flex items-start mt-3">
                                <span className="text-blue-600 dark:text-blue-400 font-bold text-xl mr-2 mt-[-2px]">•</span>
                                <p className="text-gray-800 dark:text-gray-200"><strong className="font-semibold">Manage Your Budget:</strong> Organize expenses and track funds to stay financially prepared.</p>
                            </li>
                             <Button onClick={() => navigateTo('/budget')} variant="link" className="text-blue-600 dark:text-blue-400 h-auto p-0 ml-5 text-sm lg:text-base">Go to Budget →</Button>
                        </ul>
                    </div>
                        ...
                        {loading ? (
                           <div className="flex justify-center items-center h-40 text-gray-600 dark:text-gray-400"><p>Loading Deadlines...</p></div>
                            ...
                            <div className="text-center py-6">
                                <p className="text-gray-600 dark:text-gray-400">No upcoming deadlines found.</p>
                                <Button onClick={() => navigateTo('/calendar')} size="sm" className="mt-4">
                                    View Full Calendar
                                </Button>
                            </div>
                        )}

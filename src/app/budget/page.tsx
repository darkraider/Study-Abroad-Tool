...
const BudgetSheet = () => {
  ...
  const [newCategoryType, setNewCategoryType] = useState<'expense' | 'asset'>(
    'expense'
  )
  ...
  const [addItemTargetCategory, setAddItemTargetCategory] = useState<{
    id: number
    name: string
  } | null>(null)
  ...
  const [itemCostErrors, setItemCostErrors] = useState<{
    [itemId: string]: string
  }>({})
  ...
  const handleAddItemConfirm = useCallback(async () => {
    ...
    const itemCost = parseFloat(newItemCost)
    if (!itemName) {
    ...
    try {
      ...
      if (category) {
        ...
        setExpenses(prev =>
          prev.map(exp =>
            exp.id === addItemTargetCategory.id
              ? {
                  ...updatedCategory,
        ...
        throw new Error(`Category ${addItemTargetCategory.id} not found.`)
      }
    } catch (err) {
    ...
    async (categoryId: number, itemId: string) => {
      ...
      if (costString.trim() !== '') {
        ...
        if (costValue < 0 || costValue > MAX_ITEM_COST) {
          setItemCostErrors(prev => ({
            ...prev,
            [itemId]: `$0 - $${MAX_ITEM_COST.toLocaleString()}`
          }))
          return
        }
      }
          ...
          setExpenses(prev =>
            prev.map(exp =>
              exp.id === categoryId
                ? {
                    ...updatedCategory,
                    items: updatedCategory.items.map(item => ({
                      ...item,
                      id: String(item.id)
                    }))
                  }
    ...
    <Layout backgroundImageSrc={background}>
      ...
      <div className='max-w-4xl mx-auto p-4'>
        ...
        <div className='backdrop-blur-lg rounded-xl shadow-lg p-4 md:p-6 bg-white/90 dark:bg-gray-800/90 space-y-6'>
          ...
          {error && (
            <div className='p-3 bg-red-100 dark:bg-red-900/30 border border-red-400 dark:border-red-600 text-red-700 dark:text-red-300 rounded text-center text-sm'>
              ...
              <Button
                ...
                className='ml-2 text-xs h-auto p-1 align-middle'
              >
                Dismiss
              </Button>
            </div>
            ...
            <CardContent className='p-4'>
              <div className='flex flex-col sm:flex-row gap-3 items-start'>
                ...
                <Select
                  ...
                  onValueChange={(value: 'expense' | 'asset') =>
                    setNewCategoryType(value)
                  }
                >
                  <SelectTrigger className='w-full sm:w-[150px] bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white'>
                    {' '}
          ...
          <section aria-labelledby='assets-heading'>
            <h2
              id='assets-heading'
              className='text-2xl font-semibold mb-4 text-green-700 dark:text-green-400'
            >
              Assets
            </h2>
            {expenses
              ...
              .map(category => (
                <CategoryCard
                  ...
                  onAddItem={() =>
                    openAddItemModal(category.id, category.category)
                  }
            ...
            {expenses.filter(c => c.type === 'asset').length === 0 && (
              <p className='text-sm text-gray-500 dark:text-gray-400 mb-6 ml-1'>
                No asset categories added yet.
              </p>
            )}
          </section>
          ...
          <section aria-labelledby='expenses-heading'>
            <h2
              id='expenses-heading'
              className='text-2xl font-semibold mb-4 text-red-700 dark:text-red-400'
            >
              Expenses
            </h2>
            {expenses
            ...
            {expenses.filter(c => c.type === 'expense').length === 0 && (
              <p className='text-sm text-gray-500 dark:text-gray-400 mb-6 ml-1'>
            ...
            <CardContent className='space-y-2 text-right text-base'>
              <p>
                ...
                <span className='ml-2 font-semibold text-green-600 dark:text-green-400'>
                  {formatCurrency(overallTotals.totalAssets)}
                </span>
              </p>
              <p>
                <span className='font-medium text-gray-600 dark:text-gray-300'>
              ...
              <p className='text-lg'>
                ...
                <span
                  className={`ml-2 font-bold ${
                    overallTotals.netCost >= 0
                      ? 'text-red-700 dark:text-red-400'
                      : 'text-green-700 dark:text-green-400'
                  }`}
                >
                  {formatCurrency(overallTotals.netCost)}
                </span>
              </p>
              {overallTotals.netCost < 0 && (
      ...
      >
        <DialogContent className='sm:max-w-[425px] bg-white dark:bg-gray-800'>
          ...
          <DialogFooter>
            ...
            <Button type='button' onClick={handleSaveCategoryName}>
              Save Changes
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className='sm:max-w-[425px] bg-white dark:bg-gray-800'>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              ? All items within this category will also be permanently deleted.
              This action cannot be undone.{' '}
            </DialogDescription>
          </DialogHeader>
          {deleteCategoryError && (
            <p className='py-2 text-sm text-red-500 dark:text-red-400 text-center'>
              {deleteCategoryError}
            </p>
          )}
          <DialogFooter>
      ...
      <Dialog open={isAddItemModalOpen} onOpenChange={setIsAddItemModalOpen}>
        <DialogContent className='sm:max-w-[425px] bg-white dark:bg-gray-800'>
          <DialogHeader>
            <DialogTitle className='text-gray-900 dark:text-gray-100'>
              Add Item to &quot;{addItemTargetCategory?.name}&quot;
            </DialogTitle>
          </DialogHeader>
          <div className='grid gap-4 py-4'>
  ...
  return (
    <Card className='mb-6 overflow-hidden bg-white border-gray-200 dark:bg-gray-800 dark:border-gray-700'>
      <CardHeader className='p-4 bg-gray-50/50 dark:bg-gray-700/50 flex flex-row justify-between items-center'>
        <CardTitle
          className='text-xl text-gray-800 dark:text-gray-100 flex-grow'
          title={isDefault ? 'Default Category' : 'Category Name'}
        >
          {category.category}
          ...
          {!isDefault && (
            <>
              <Button
                ...
                variant='ghost'
                size='icon'
                className='h-7 w-7 text-gray-600 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400'
              ...
              <Button
                ...
                variant='ghost'
                size='icon'
                className='h-7 w-7 text-gray-600 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400'
      ...
      <CardContent className='p-0'>
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className='pl-4 w-2/5 text-gray-600 dark:text-gray-300'>
                Item
              </TableHead>
              <TableHead className='w-1/4 text-right text-gray-600 dark:text-gray-300'>
                Cost ($)
              </TableHead>
              <TableHead className='w-1/4 text-center pr-4 text-gray-600 dark:text-gray-300'>
                Action
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {(Array.isArray(category.items) ? category.items : []).map(item => (
              <TableRow key={item.id}>
                <TableCell className='pl-4 font-medium text-gray-900 dark:text-gray-100'>
                  {item.item}
                </TableCell>
                <TableCell className='text-right'>
                  <div className='inline-block relative'>
                    <Input
                      ...
                      className={`w-28 text-right h-8 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white ${
                        itemCostErrors[item.id]
                          ? 'border-red-500 dark:border-red-400 focus:ring-red-500'
                          : 'border-gray-300 dark:border-gray-600'
                      }`}
                      aria-label={`Cost for ${item.item}`}
                      aria-invalid={!!itemCostErrors[item.id]}
                      aria-describedby={
                        itemCostErrors[item.id]
                          ? `item-error-${item.id}`
                          : undefined
                    ...
                    {itemCostErrors[item.id] && (
                      <p
                        ...
                        className='absolute text-xs text-red-500 dark:text-red-400 mt-1 right-0'
                      >
                        {itemCostErrors[item.id]}
                      </p>
                    )}
                ...
                <TableCell className='text-center pr-4'>
                  <Button
                    ...
                    variant='ghost'
                    size='icon'
                    className='text-red-500/80 hover:text-red-600 hover:bg-red-100 dark:hover:bg-red-900/20 h-7 w-7'
                    title='Remove Item'
                  >
            ...
            <TableRow>
              <TableCell colSpan={3} className='pt-3 px-4 pb-3'>
                <Button
                  variant='outline'
                  size='sm'
                  className='w-full text-gray-700 border-gray-300 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700'
                  onClick={onAddItem}
                >
